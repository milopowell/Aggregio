...
def strava_callback():
    ...
    user = User.query.get(token_info['athlete']['id'])
    if user:
...
def finalize_aggregate():
    ...
    if request.method == 'POST':
        ...
        total_stats, type_stats, map_data_list = {'distance': 0, 'moving_time': 0, 'total_elevation_gain': 0}, {}, []
        for act_id in activity_ids:
            ...
            if act_response.status_code == 200:
                ...
                if act_type not in type_stats: type_stats[act_type] = {'distance': 0, 'moving_time': 0, 'count': 0}
...
def view_single_aggregate(aggregate_id):
    ...
    return render_template('view_aggregate.html', aggregate=aggregate, map_data=map_data, mapbox_token=MAPBOX_TOKEN, meters_to_miles=meters_to_miles, meters_to_feet=meters_to_feet, seconds_to_hms=seconds_to_hms)
